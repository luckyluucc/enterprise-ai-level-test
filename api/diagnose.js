...
# 你的最大卡点
（一段话，80-180 字。呼应 D1 用户原话，用诊断口吻回应。**不要复读用户的话**——要点出他没看到的深层卡点，比如"你说员工不肯用，本质是 KPI 没绑定"。要尖锐、要直接，不要"很常见""完全理解"这种安慰话。）

# 跨到 Lv.${lv === 10 ? 10 : (lv + 1)} 的 3 个具体动作
${lv === 10 ? '（这一段写：你已经在 Lv.10。给 3 个"保持不倒退"的具体动作。）' : `（3 个动作，每个动作必须满足：① 一周内能开始 ② 不需要再招人 ③ 跟他的行业/规模匹配。每个动作 1-2 句话。用 \`1. **动作名**：说明\` 的 markdown 列表格式。）`}
